...
\item The cost per W of installed PV capacity and the cost per Whr of installed battery capacity. 
\end{itemize}
...
The optimization algorithm calls upon the energy balance algorithm, which in turn computes the LEGP. For each iteration, $P_{nom}$ is increased by $P_{step}$ e.g.(100W). 
When the optimization algorithm calls upon the energy balance algorithm, it specifies the initial charge state of the battery bank to be $E_{Bmin}$.
\begin{verbatim}

** First Step **
- For a desired LEGP, find the smallest PV module
 which will achieve the LEGP given a huge battery bank i.e(100x peak electricity 
 demand)
		Given the finite PV and battery increments the actual LEGP will be 
		slightly better than the LEGP specified

** Second Step **  
- Increase the PV capacity by the fixed increment. Then find the smallest 
  Battery Bank which satisfies the Desired LEGP
  
** Third Step ** 
- Calculate the cost of each combination based on the 
  fixed cost of PV and Battery

** Fourth Step **
-Save the minimum cost option

** Fifth Step **
- Compute last expensive battery/PV combination for all other 
 desired LEGPs. Produce a plot of Cost vs LEGP 
\end{verbatim}
